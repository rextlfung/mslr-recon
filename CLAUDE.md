...
7. Compute or reuse Lipschitz constant `L = Nscales × σ₁(A)²`. If `σ1A` isn't precomputed, `σ₁(A)` is estimated via `poweriter_frames` (`src/mirt_mod.jl`), which power-iterates each per-frame operator `Aframes[t]` independently and takes the max — see [Optimizer](#optimizer--pogm_restart--poweriter-srcmirt_modjl) below for why this is exact, not an approximation.
...
11. Save output to `fn_recon` (the caller-specified full path). Persisted keys include the recon (`X`, `X_recon`, `omega`), per-iteration traces (`dc_costs`, `reg_costs`, `restarts`, `rel_changes`), parameters (`R`, `sigma1A`, `sigma1_per_frame`, `L`, `Nscales`, `patch_sizes`, `strides`, `lambdas`, `lambda_global`, `Niters`, `mom`, `conv_tol`, `cycle_spin`), and runtime metadata (`used_gpu`, `device`, `runtime_s`, `iter_time_s`). `sigma1_per_frame` is empty (`Float64[]`) when `σ1A` was passed in precomputed rather than measured this run. See README.md for the full key reference. Additionally, `abs.(X_recon)` (magnitude, `Float32`) is written to a NIfTI file at the same path with `.mat` replaced by `.nii.gz`, for direct viewing in fsleyes/SPM/AFNI — the complex phase is discarded (only `.mat` retains full complex `X`/`X_recon`).
...
**`σ₁(A)` estimation is per-frame, not on the full stacked operator.** `A = block_diag(Aframes...)` is block-diagonal over time frames, and the spectral norm of a block-diagonal operator equals the max over its blocks' spectral norms exactly — `‖block_diag(A_1,...,A_n)‖ = max_t ‖A_t‖`, attained by a vector supported entirely on the argmax block (Cauchy–Schwarz-style: `‖Ax‖² = Σ_t ‖A_t x_t‖² ≤ (max_t ‖A_t‖²) Σ_t ‖x_t‖²`, with equality when `x` is the top singular vector of the worst block, zero elsewhere). `poweriter_frames` (`src/mirt_mod.jl`, called from `reconstruct.jl` §6) exploits this: it power-iterates each `Aframes[t]` independently — each `Nt` times cheaper per iteration than the full stack — and returns `(σ1s, maximum(σ1s))`. This is *exact*, not an approximation, and additionally surfaces the frame-to-frame spread (`σ1s`, saved as `sigma1_per_frame`). On CPU, frames run concurrently via `Threads.@threads` (same no-shared-mutable-state assumption as the `dc_cost`/`dc_cost_grad` threading); CuArrays can't use `@threads`, so GPU frames run sequentially — still far cheaper than power-iterating the `Nt`-times-larger stacked operator.
...
**`σ₁(A) ≤ 1.0`** always (subsampling can only reduce the norm of the unsubsampled unitary operator). The unsubsampled operator is exactly unitary (σ₁ = 1), but subsampling with an incoherent mask reduces it slightly. Empirically, `σ₁(A) ≈ 0.968` for the 20260409tap dataset (measured via `tests/sigma1A_tests.jl`). Hard-code `σ1A` after the first run to skip re-measuring it (now a per-frame `poweriter_frames` call — see [Optimizer](#optimizer--pogm_restart--poweriter-srcmirt_modjl) — cheap enough that the old "~20 min" full-stack cost no longer applies, but the result is dataset-specific so caching it is still worthwhile). Using 1.0 is safe (overestimates L, so step size is conservative) but ~6.7% suboptimal.
